...
with header_col1:
    ...
    st.markdown("""
    This tool predicts antimicrobial resistance (AMR) from genetic sequences, identifies resistance genes, 
    translates them to proteins, and recommends effective antibiotics.
    """)
    st.markdown('<div class="loading-animation"></div>', unsafe_allow_html=True)
with header_col2:
    if dna_animation:
...
with st.sidebar:
    ...
    with sidebar_tab2:
        ...
        try:
            ...
            st.warning("Could not load analysis history. The database might not be ready yet.")
            st.info("Try running your first analysis to initialize the database.")
